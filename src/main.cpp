...
/*
	...
	- plot stuff (need to separate all the data into 1-line vectors)
		...
		- position (elements 6-> of the state vector, x)
...
int main()
{
	...
	// For dt = 0.2
		...
		for (int i = 0; i < timeVector1.size(); i++)
		{
			phi_dot[i] = X_totalData[i][0];
			theta_dot[i] = X_totalData[i][1];
			psi_dot[i] = X_totalData[i][2];
		}
		//

	/* Example showing how to run it with different dt values (keeping old data instead of running it again with diffferent dt)
	
	// For dt = 0.1
		NumericalIterator.change_dt(0.1);
		NumericalIterator.run();
